...
- **A/B 股分红送股公告**：Tushare `dividend` 接口（`div_proc="实施"` 白名单过滤），
  同步为"分红建议"待用户确认入账，绝不自动写公司行动；判重窗口放宽以匹配
  券商导入记录（其 ex_date 实为到账日、金额常为税前全额）。
- **标的未来事件**：财报披露计划（`disclosure_date`）、分红预案（`dividend`
  非实施行）、限售解禁（`share_float`）落 `security_events` 全局表，持仓页角标展示。
- **港股/美股无公告接口**：分红仍以券商对账单导入为准（IBKR/东财/招商）。
- 以上三接口已用真实 token 验证连通（2026-08-01）；`div_proc` 实测取值：
  实施 / 股东大会通过 / 股东提议 / 预案。
- **基准指数**：`index_daily`（沪深300）与 `index_global`（恒生 HSI / 标普 SPX）
...
2. **IBKR**：下载完整 Activity Statement，CSV 用于导入、PDF 用于人工核验；
   后续再考虑 Flex Query 自动化。
...
| IBKR         | Activity Statement CSV + PDF                      | 交易、期末持仓、各币种现金、出入金、股息、利息、费用、税和公司行动 | Transaction History CSV；转化股票/ETF 买卖、股息、外国预扣税、少量已知转板，以及存款/取款、贷/借方利息、外汇兑换（两腿+佣金）的现金入账 | 期权与`调整`（纸面损益）只归档不入账；Flex 尚未接入                                              |
...
- 招商入口为 PDF-only（电子对账单解密工作副本）；所有券商文件的预览与正式
  导入都必须选择匹配账户，以保证账户级去重和核对。
- "解析成功"不等于"账户已对平"：正式导入前需补齐期初持仓或取得更早历史
  文件；无法确认成本或行语义时，不猜测生成买入交易。
